...
char *cp;
{
    ...
    if (!cp || !*cp)
	return tl;
    while ((i = *cp++) != '\0')		/* Stop when see zero token */
	{
	if (i < NTOKDEFS)
	    {
	    pptok = zpptok;
...
static
char *
tltostr(tlist_t tl, char *cp, int n)
{
    register int i;
    register char *str;
...
SYMBOL *sym;
{
...
#if DEBUG_PP
    if (debpp)
	fprintf(fpp,"mexpsym(%#lo->\"%s\", %d)\n", (INT) sym, sym->Sname, hs);
#endif
    ...
    */
...
#if __MSDOS__ /* KAR-8/92, predefine __KCCDOS__ for KCCDOS only */
	    case MACF_DOS:
#endif
	...
	cp1 = (char *) sbuf;	/* Must copy string lits */
...
tlist_t il;
{
	...
	switch (nextmacpp())
		...
		switch (rawptr->pt_is)
			...
			if ((sym = findmacsym(rawval.cp)) == NULL)
			    {
...
if (il.tl_head->pt_typ == T_EOF)	/* If all gone, */
    ...
    (il.tl_tail = tl.tl_tail)->pt_nxt = 0;	/* remove the EOF */
    ...
    tl = ml;		/* TL truncated to empty, just use ML now */
    ...
    tl = il;		/* Nothing on TL, just use IL */
...
*mf, int hs)
{
	...
	switch (typ)
		...
		switch (typ)	/* Handle arg as directed */
		    {
		    case T_MACSTR:
			argl = tlstrize(argl);   break;	/* Stringize */
		    case T_MACINS:
			argl = tlcopy(argl);     break;	/* Insert */
		    case T_MACARG:
			argl = mexplim(tlcopy(argl), hs); break;	/* Expand */
		    default:
			int_error ("msubst: invalid macro %d type", typ);
		    }
...
PPTOK *p;
{
    ...
    if (p->pt_typ == T_IDENT)
	{
	if ((sym = findmacsym(p->pt_val.cp)) == 0)
...
*mf;
{
    ...
    fprintf(fpp,
"%s%s: Macframe for %#lo->\"%s\" (npar: %d, parlen: %d, len: %d)\n",
		ind, str, (INT) s, s->Sname, s->Smacnpar, s->Smacparlen,
		i = s->Smaclen);
    fprintf(fpp,"%sBody: %#lo-> \"", ind, (INT) cp = s->Smacptr);
    if (cp) while (--i >= 0)
...
break;
    ...
    default:
	if (res != 3)
	    int_error ("directive: invalid res %d", res);
	}
...
d_define (void)
    ...
    /* Look for formal-parameter names */
     ...
     *
     * then the comment is going to be detected and abolished by this
     * call to nextrawpp ().  nextrawpp () does not know who called it;
     * left to its own devices, it will return the first token of the
     * next non-empty line.
     */
	...
	while (1)
		...
		*/
		...
		for (i = 0; i < m.mf_nargs; i++)	/* Scan to find dups */
			...
			{
			error ("Duplicate formal parameter in macro def; \"%s\"",
			       rawval.cp);
			break;
			}
		    }
	...
	if (rawpp != T_RPAREN)
	    ...
	    nextrawpp();		/* Set up next token */
	}

    /*
     * At this point, we may have parsed a list of formals, or there
     * may simply have been no formals at all.
     */
...
break;
    ...
    */
    if ((cp = mf->mf_body = calloc(1, mf->mf_len+1)) == NULL)
	{
...
for (;;)
	...
	switch (ch)	/* Scan starting with current char */
		...
		else if (ch == '/' && !clevnocpp) /* FW 2A(46) August 1993 */
		    {			/* " */
		    comment_type = CPP;	/* " */
		    scancomm ();	/* " */
		    comment_type = C;	/* " */
...
if, ifdef, ifndef, elif,
    ...
    , and endif.  ifdef and ifndef are merely
    variants of #if.
...
the setting is always kept at in-if.  This allows both elif and
else
    to
...
d_elif()
{
	...
	return d_if();				/* Handle as if plain #if */
	}
...
#if 0			/* 8/91 #pragma eof */
    if (eof_pragma)	/* match (pop off) each #if, #ifdef, #ifndef*/
	{
	while (iflevel && iffile[iflevel] == 0)
	    (void) d_endif ();
    eof_pragma = 0;
	}
#endif
...
*/
static
int cinctry(int, char **, char *, char *, FILE **);
#if 0
static
int cinctry();	/* Auxiliary just for cinclude() */
#endif
...
d_include (void)
{
    ...
    /* 11/91 #pragma include_once uses find_file() to avoid 2nd include */
    #define find_file(f)    (nsert_file((f),0))
    if ((ftype = getfile(f, FNAMESIZE)) == 0 || find_file(f))
	return PPR_FLUSH;	/* If failed to get name, ignore */

    /* A filename starting with '/' (ie an absolute pathname)
    ** tries just that name; nothing else would make sense.
    */
    if (*f == '/')
	{
...
d_line(void)
{
    ...
    if ((pint = tlpcur(tl)) == NULL)
	return error("Empty #line"), PPR_CHECKEOL;
    if (pint->pt_typ != T_ICONST)
	return error("Bad #line number"), PPR_CHECKEOL;
    if ((pstr = tokn2p(pint->pt_nxt)) != NULL)
	{
	if (pstr->pt_typ != T_SCONST)
	    return error("Bad #line filename"), PPR_CHECKEOL;
	if (pstr->pt_nxt)
	    return error("Bad #line syntax"), PPR_CHECKEOL;
	}
    ...
    if (*pstr->pt_val.cp == 'L')
	{
	error("#line directive string literal cannot be wide char");
	    pstr->pt_val.cp++;
	}
 ...
 *		to turn high-level debugging and profiling on and off.
 *		This is primative and not meant to be general, but must
 *		not be broken by a future more general implementation.
...
d_pragma (void)
{
    ...
    if (strncmp(pragma_str, " debug", 6) == 0)
	debcsi = (debcsi >= 0) ? debcsi : -debcsi;	/* make positive */
else if (strncmp(pragma_str, " nodebug", 8) == 0)
    debcsi = (debcsi <= 0) ? debcsi : -debcsi;	/* make negative */
else if (strncmp(pragma_str, " message", 8) == 0)
	/* 11/91 prints a string without the halting of a #error */
    fprintf (stdout, "%s\n", &pragma_str[8]);
else if (strncmp(pragma_str, " request_library", 16) == 0 &&
    get_title (pragma_str, &ext))
    {
	/* 11/91 adds a .REQUEST  LIB*.REL to the .MAC file */
    void outnl (void);
    outstr ("\t.REQUEST ");
    outstr (title);
#define fnxisREL(s) ((s)[0]=='.' && toupper((s)[1])=='R' &&toupper((s)[2])=='E' && toupper((s)[3])=='L')
    if (!fnxisREL(&title[ext]))
	outstr (".REL");
    outnl();
    }
	...
	get_title(pragma_str, &ext))
    {
    ungetc(ch, in);		/* getlinetl() above ate 1 char too many */
    ch = ' ';		/* getlinetl() above got 1 ch too many */
    module_pragma = 1;	/* only place that turns on module_pragma */
    }
#if 0 /* KAR-8/92, removed note, but kept source around for later use in, */
      /* possibly, a new class of messages to the user, or such */
else
    note("Unknown #pragma %s", pragma_str);
#endif
return PPR_CHECKEOL;
}
...
get_title (char *pragma_str, char *ext)
{
	...
	if (pragma_str[i] == '(')
	{
	for (j = 0; pragma_str[i+j+1] != ')' && j < TITLE_SIZE; j++)
	    title[j] = pragma_str[i+1+j];
	title[j] = '\0';
	*ext = j - 4;
	return 1;
	}
...
while (--i > 0)
    ...
    if (isceol(ch) || ch == EOF)
	{
	error("missing %c in include file spec", type);
...
cskiplwsp (void)
	...
	switch (ch)
		...
		if (nextch () != '*')
		    {
		    /*
		     * This is not a C comment.  It may be a C++
		     * comment, or it may not be a comment at all.
		     */

		    if (ch == '/' && !clevnocpp) /* FW 2A(46) August 1993 */
			comment_type = CPP;	/* FW 2A(46) August 1993 */
		    else			/* FW 2A(46) August 1993 */
			{
			pushch (ch);
			return pushstr ("/");
			}
		...
		if (comment_type == CPP)/* FW 2A(50) September 1993 */
		    pushch (ch);	/* FW 2A(50) September 1993 */

		comment_type = C;	/* FW 2A(46) August 1993 */
		nextch ();
		break;
...
checkeol(void)
{
	...
	default:
		...
		error("Non-whitespace following directive");
else
...
static
tlist_t
tlwspdel(tlist_t tl, int allf)		/* TRUE to flush all whitespace */
{
    register PPTOK *p, *lastp = NULL;
...
{
    typedef char (* file_list_t)[FILE_LEN];
    static file_list_t file_list = NULL;
    int i = 0;

    if (!f)		/* 2/92 free previous list if #pragma module (title)*/
	{
	if (file_list)
	    free(file_list);
	file_list = NULL;
    ...
    {
    for (; file_list[i][0] != '\0'; i++)
      if (strncmp(f, file_list[i], FILE_LEN) == 0)
	  return 1;	/* file with #pragma include_once already included */
    }
else
    {
    file_list = (file_list_t) malloc (FILE_MAX * FILE_LEN);
    if (file_list == NULL)
	jerr("Out of memory for file list\n");
    file_list[0][0] = '\0';   /* ensure that initial file_list[][0] is zero */
    }

/* #pragma include_once sets insert_flag  and  -i flag sets insert_all_files 
 * Insert first 127 *.H files (remaining files may be inserted twice or more)
 */
if ((insert_flag || insert_all_files) && i < FILE_MAX - 1)
    {
    strncpy (file_list[i], f, FILE_LEN);
    file_list[i+1][0] = '\0';   /* ensure that next file_list[][0] is zero */
    }
