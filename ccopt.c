	...
	switch (p->Pop & POF_OPCODE)
{
	...
	** we recognize, only three things (MOVEI, MOVE, ADJBP) should
	** be able to generate such addresses.
	*/
	if (stackrefs)
	    {
	    if ((p->Pop&POF_OPCODE) == P_MOVE
	      && ((p->Ptype&PTF_IMM)				/* MOVEI */
...
killstack()
    ...
    if (stackrefs != 0)
	return;		/* funny stack usage, give up */
    for (i = 0; i <= MAXSTACK; i++)
	stkused[i] = 0; /* no stack usage yet */
	...
	switch (p->Pop & POF_OPCODE)
{
	...
	*/
	i = - (int) p->Poffset;		/* get negated offset from stack */
	if (i < 0 || i > MAXSTACK)
	    i = MAXSTACK; /* fold range */
	...
	if (!(p->Ptype & PTF_IND))
	    ...
	    */
	    ...
	    /* Found a reg with mem value already in it, so use register op */
		    ...
		    if ((q = before(p)) != NULL && isskip(q->Pop))
			p->Ptype |= PTF_SKIPPED;
    ...
    switch (p->Pop)
{
    ...
    case P_SETZ:
    case P_SETO:
	if ((p->Ptype &~ PTF_SKIPPED) != PTA_ONEREG)
	    return 0;
